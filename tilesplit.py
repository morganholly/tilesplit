		...
		if len(line) <= 1:
			...
			try:
				if len(line_split) > 5:
					...
					append = ""
				out_list.append(("region", regions[line_split[1]], (int(line_split[2]) * scale, int(line_split[3]) * scale), line_split[4], append, scale))
				...
				# region_list.extend(expand_region(regions[line_split[1]], ((int(line_split[2]) + offset[0]) * scale, (int(line_split[3]) + offset[1]) * scale), line_split[4], append, verbose))
			except KeyError:
